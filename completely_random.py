...
for i in range(num_files):
  ...
  bfs = [0] # contains actual indices of nodes, not their parsing indices
  parse_index = [0] # parse index is the index in the array of parsing, par is already an index for parsing, 
  # children is also an index for parsing
  real_par = [-1] # the actual parent, not the parsing parent
  total_occ = [0] * 7 # total number of occurences of a container
  total_occ[0] = 1
  myorder = [1] 

  lout.write("Create an ARXML file that includes CanNm module with CanNmGlobalConfig" + "\n")
  ...
  while j < len(bfs):
    ...
    for v in container_parameters[k]:
      res_value = -1
      create = 0
      if str(v.lower_mult) == "1":
        create = 1
      else:
      ...
      if str(v.data_type) == 'INTEGER':
        ...
        if create_2 != 0:
          create_2 = random.randint(0, 1)
        if create_2 == 0:
          res_value = get_default_value(v.data_type)
          
        if res_value != get_default_value(v.data_type) or str(v.lower_mult) == "0":
        ...
        if abs(res_value - get_default_value(v.data_type)) > 0.000000001 or str(v.lower_mult) == "0":
          lout.write(parameter_sentence(container_def[k].name, v.def_name, str(res_value), theorder) + "\n")
        ...
        if create_2 != 0:        
          if which == 0:
            res_value = "False"
            if str(v.lower_mult) == "0":
              lout.write(parameter_sentence(container_def[k].name, v.def_name, "False", theorder) + "\n")
          else:
            res_value = "True"
            lout.write(parameter_sentence(container_def[k].name, v.def_name, "True", theorder) + "\n")
        else:
          res_value = "False"
          if str(v.lower_mult) == "0":
            lout.write(parameter_sentence(container_def[k].name, v.def_name, "False", theorder) + "\n")

      elif str(v.data_type) == 'ENUMERATION':
        ...
        if res_value != enumer_value[0] or str(v.lower_mult) == "0":
          lout.write(parameter_sentence(container_def[k].name, v.def_name, str(res_value), theorder) + "\n")
