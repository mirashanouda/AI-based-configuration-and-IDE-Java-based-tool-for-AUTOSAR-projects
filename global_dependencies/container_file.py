...
class container(tag_file.tag):
  ...
  def add_parameter(self, param):
    if self.has_param_val == 0:
      ...
      self.param_val_idx = len(global_vars.et_elements)
      global_vars.et_elements.append(ET.Element("PARAMETER-VALUES"))
      global_vars.et_elements[self.et_element_idx].append(global_vars.et_elements[self.param_val_idx])
    global_vars.et_elements[self.param_val_idx].append(global_vars.et_elements[param.et_element_idx])

  def add_child(self, child):
    if self.has_sub_cont == 0:
      self.has_sub_cont = 1
      self.sub_cont_idx = len(global_vars.et_elements)
      global_vars.et_elements.append(ET.Element("SUB-CONTAINERS"))
      global_vars.et_elements[self.et_element_idx].append(global_vars.et_elements[self.sub_cont_idx])
    global_vars.et_elements[self.sub_cont_idx].append(global_vars.et_elements[child.et_element_idx])
