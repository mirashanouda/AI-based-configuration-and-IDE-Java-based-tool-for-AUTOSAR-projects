...
static inline void CanNm_Internal_SetPduCbvBit( const CanNm_ChannelType* ChannelConf, const uint8_t PduCbvBitPosition );
static inline void CanNm_Internal_ClearPduCbvBit( const CanNm_ChannelType* ChannelConf, const uint8_t PduCbvBitPosition );
...
static inline uint8_t CanNm_Internal_GetUserDataOffset( const CanNm_ChannelType* ChannelConf );
 ...
 */
Std_ReturnType CanNm_CheckRemoteSleepInd(NetworkHandleType nmChannelHandle, bool* nmRemoteSleepIndPtr)
{
	CanNm_Internal_ChannelType* ChannelInternal = &CanNm_Internal.Channels[nmChannelHandle];
...
void CanNm_RxIndication(PduIdType RxPduId, const PduInfoType* PduInfoPtr)
{
	...
	if (ChannelConf->PduCbvPosition != CANNM_PDU_OFF && ChannelConf->NodeDetectionEnabled) {
		uint8_t cbv = PduInfoPtr->SduDataPtr[ChannelConf->PduCbvPosition];
